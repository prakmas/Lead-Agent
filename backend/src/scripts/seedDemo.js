...
// Sample supply records the matching engine can score leads against.
const demoListings = [
  {
...
// Sample inbound messages routed through the real pipeline so that
// conversations, leads, matches, and messages all populate the dashboard.
const demoMessages = [
  {
    channel: "whatsapp",
  ...
  for (const message of demoMessages) {
    ...
    console.log(
      `Processed ${message.channel} lead from ${message.contactName}: ` +
        `${result.matches.length} match(es), reply -> ${result.reply.slice(0, 60)}...`,
    );
  }
